...
.get(async (req,res) =>{
    ...
    if(month===null && year===null){
        userAttendanceRecords=await attendanceRecordModel.find({user:token.id})
    }
    else{
        if(month>=0 && month<11){
...
.get(async (req,res) =>{
    ...
    hours=getMissingAndExtraHours(month,year,userAttendanceRecords,convertDayOff(user.dayOff));
    try {
        res.send(hours);
    }
    catch (error) {
